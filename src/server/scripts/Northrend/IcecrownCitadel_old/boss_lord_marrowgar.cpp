...
enum Spells
{
   ...
   SPELL_BONE_SPIKE_GRAVEYARD_10_NORMAL =  69057,
    SPELL_BONE_SPIKE_GRAVEYARD_25_NORMAL =  70826,
        SPELL_BONE_SPIKE_GRAVEYARD_25_HEROIC =  72088,
...
//            npc_bone_spikeAI(Creature *pCreature) : Scripted_NoMovementAI(pCreature), vehicle(pCreature->GetVehicleKit())
            ...
            if (BoneSpikeGUID)
            {
                Unit* pBone = Unit::GetUnit((*me), BoneSpikeGUID);
                if (pBone)
                    pBone->RemoveAurasDueToSpell(SPELL_SPIKE_IMPALING);
            }
            }
...
//                m_uiMoveTimer = 5000;
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                for (uint8 i = 1; i <= m_uiBoneCount; i++)
                {
                        ...
                        {
                                Creature* Bone = me->SummonCreature(CREATURE_BONE_SPIKE, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ()+5, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 999999);
                                CAST_AI(npc_bone_spike::npc_bone_spikeAI, Bone->AI())->SetPrisoner(pTarget);
                                Bone->CastSpell(pTarget, SPELL_SPIKE_IMPALING, true);
                                DoScriptText(RAND(SAY_SPIKE_1,SAY_SPIKE_2,SAY_SPIKE_3), me);
                        }
                }
//                        m_uiBoneSpikeGraveyardTimer = 120000;
