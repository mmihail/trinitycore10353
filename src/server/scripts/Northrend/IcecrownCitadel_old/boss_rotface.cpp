...
enum Spells
{
    ...
    SPELL_MUTILATED_INFECTION10 = 69674,
    SPELL_MUTILATED_INFECTIONH10 = 73022,
    SPELL_MUTILATED_INFECTION25 = 71224,
    SPELL_MUTILATED_INFECTIONH25 = 73023,
};
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (m_uiSlimeSprayTimer <= diff)
                {
                    if(Unit* pTarget = SelectUnit(SELECT_TARGET_RANDOM, 1))
                    {
                        ...
                        Creature* trigger = me->SummonCreature(CREATURE_OOZE_SPRAY_STALKER, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN, 4000);
                        if(trigger)
                        {
...
//                            DoCast(pTarget, SPELL_INFECTION_AURA);
                            DoCast(pTarget, RAID_MODE(SPELL_MUTILATED_INFECTION10,SPELL_MUTILATED_INFECTIONH10,SPELL_MUTILATED_INFECTION25,SPELL_MUTILATED_INFECTIONH25));
//                        }
                    }
                    m_uiInfectionTimer = 14000;
